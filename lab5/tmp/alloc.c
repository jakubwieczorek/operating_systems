...
{
	struct hole* act;
	unsigned int buffer_size; /* in items not in pairs*/
	int counter;
	char* p;
	phys_clicks* buffer;	

	buffer_size = mm_in.m1_i1 / sizeof(unsigned int); /* 4096 / 4*/
	
        for(act = hole_head, counter = 0; act != NIL_HOLE && counter < buffer_size; act = act->h_next)
	{	
		buffer[counter++] = act->h_len;
		buffer[counter++] = act->h_base;
	}

	buffer[counter] = 0;
	
	/*      SRC_pid, SRC_type, SRC_buffer        , DST_pid       , DST_type, DST_buffer             , nbytes  */
