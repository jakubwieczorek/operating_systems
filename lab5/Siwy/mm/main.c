...
int do_getprocnr(void)
{
  ...
  if(proc_id>=0)
  {
    for(; i < NR_PROCS; i++)
    {
      mpr=&mproc[i];
      fl=mpr->mp_flags;
      if((fl & IN_USE) && mpr->mp_pid==pid)
      {

	mpr->mp_reply.m1_i1=i;
	return(i);
      }	
     }
     return ENOENT;
...
int do_setgroup(void)
{
  _taskcall(SYSTASK,SYS_SETGROUP,&mm_in);
}

int do_settime(void)
{
  _taskcall(SYSTASK,SYS_SETTIME,&mm_in);
}

int do_sinfo(void)
{
 _taskcall(SYSTASK,SYS_SCHEDINFO,&mm_in);
}
