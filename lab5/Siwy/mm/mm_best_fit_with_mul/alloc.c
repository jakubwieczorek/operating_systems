 ...
 */
  ...
  if(another==1)
  {
	register struct hole * min_hole, * prev_min_hole;
	hp=hole_head;
  	if(hp!=NIL_HOLE)
 	{
		min_hole=NIL_HOLE;
		prev_min_hole=NIL_HOLE;
	}	  
	else
		return(NO_MEM);
	
	/* wyszukiwanie najmniejszej dziury*/
	while(hp!=NIL_HOLE && hp->h_base < swap_base)
	{
		if((hp->h_len < min_hole->h_len || min_hole==NIL_HOLE) && hp->h_len > mul*clicks)
		{
			min_hole=hp;
			prev_min_hole=prev_ptr;
		}
		prev_ptr=hp;
		hp=hp->h_next;
	}
	if(min_hole->h_len>mul*clicks)
	{
		old_base=min_hole->h_base;
		min_hole->h_base+=clicks;
		min_hole->h_len-=clicks;
		if(min_hole->h_len==0 && prev_min_hole!=NIL_HOLE)  del_slot(prev_min_hole,min_hole);
		return (old_base);
	}
	else
		flag=1;
  }
  else if(another==0 || flag==1);
  {
  	do 
	{
		hp = hole_head;
...
int do_hole_map()
{
	...
	while(hp!=NIL_HOLE && hp->h_base < swap_base && i<max);
	buffer[i]=0;
